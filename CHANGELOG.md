...
- Bounded CI real-data streaming execution with an explicit job timeout and lower network retry envelope (`timeout 15m`, `--network-timeout-s 45`, `--max-network-attempts 1`) to convert cancellation-prone runs into clear pass/fail outcomes.
