...
### Changed
- Reduced routine GitHub Actions load by validating committed real-data docs assets on pull requests while running networked real-data refresh only on manual `workflow_dispatch`; this preserves hard missing-asset failures without forcing heavy geospatial streaming on every PR.
